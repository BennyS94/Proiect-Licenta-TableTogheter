...
export function CaloriesFlameIcon({
  color = "#1F252D",
...
export function ProteinDrumstickIcon({
  ...
  return (
    <Image
      ...
      style={[{ height: size, width: size }, color ? { tintColor: color } : null]}
    />
  );
}

export function CarbsWheatIcon({ color, size = 32 }: MacroNutrientIconProps) {
  return (
    <Image
      accessibilityIgnoresInvertColors
      resizeMode="contain"
...
export function FatsAvocadoIcon({ color, size = 32 }: MacroNutrientIconProps) {
  return (
    <Image
      accessibilityIgnoresInvertColors
      resizeMode="contain"
