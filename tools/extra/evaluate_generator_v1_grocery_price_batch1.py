...
CATALOG_PATH = ROOT / "data/grocery/draft/grocery_product_catalog_v1_batch1.csv"
BATCH_INPUT_PATH = ROOT / "data/grocery/draft/grocery_product_catalog_v1_price_research_filled_batch1.csv"
AUDIT_DIR = ROOT / "data/grocery/audit"
SUMMARY_OUT = AUDIT_DIR / "grocery_price_batch1_demo_summary.txt"
ITEMS_OUT = AUDIT_DIR / "grocery_price_batch1_demo_items.csv"
SAMPLE_OUT = AUDIT_DIR / "grocery_price_batch1_demo_sample.txt"
...
def main() -> None:
    ...
    if not isinstance(pricing_summary, dict):
        pricing_summary = {}
    print("Generator v1 Grocery price batch1 audit written")
...
def build_demo_grocery_list() -> dict[str, Any]:
    ...
    grocery_list["round61_plan_summary"] = plan.get("multi_day_summary", {})
    return grocery_list
...
def build_summary(
    *,
    ...
    if not isinstance(pricing_summary, dict):
        pricing_summary = {}
    plan_summary = grocery_list.get("round61_plan_summary", {})
    if not isinstance(plan_summary, dict):
        plan_summary = {}
    catalog_status_counts = _counts(batch.get("decision_status", []))
    priced_rows = [row for row in item_rows if _clean_text(row.get("estimated_cost"))]
    missing_rows = [
        row for row in item_rows
        if _clean_text(row.get("price_warning"))
        and not _clean_text(row.get("estimated_cost"))
    ]
    lines = [
        "Generator v1 Grocery price batch1 demo audit",
        "",
        ...
        "",
        f"catalog_rows_safe={len(catalog)}",
