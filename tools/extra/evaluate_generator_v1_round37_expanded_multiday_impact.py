...
ROUND37_CACHE = REPO_ROOT / "data" / "recipesdb" / "draft" / "recipes_v1_2_round37_plus100_nutrition_cache.csv"

OUT_SUMMARY = AUDIT_DIR / "generator_v1_round37_expanded_multiday_impact_summary.txt"
OUT_DAYS = AUDIT_DIR / "generator_v1_round37_expanded_multiday_days.csv"
OUT_MEALS = AUDIT_DIR / "generator_v1_round37_expanded_multiday_meals.csv"
OUT_REPETITION = AUDIT_DIR / "generator_v1_round37_expanded_multiday_repetition.csv"
OUT_CANDIDATE_POOL = AUDIT_DIR / "generator_v1_round37_expanded_multiday_candidate_pool.csv"
...
def main() -> None:
    ...
    round37_ready_ids = load_round37_ready_recipe_ids()
    rows = []
    day_rows = []
    meal_rows = []
    repetition_rows = []
    ...
    for scenario in SCENARIOS:
        ...
        rows.append(summary_row(scenario, plan, diagnostics, round37_ready_ids))
        day_rows.extend(day_output_rows(scenario, plan, round37_ready_ids))
        meal_rows.extend(meal_output_rows(scenario, plan, round37_ready_ids))
        repetition_rows.extend(repetition_output_rows(scenario, plan))
        pool_rows.append(candidate_pool_row(scenario, plan, diagnostics))
    ...
    pd.DataFrame(pool_rows).to_csv(OUT_CANDIDATE_POOL, index=False)
    OUT_SUMMARY.write_text(build_summary_text(rows, round37_ready_ids), encoding="utf-8")

    expanded = next(row for row in rows if row["scenario"] == "round37_expanded")
    print("Generator v1 Round37 expanded dataset impact evaluation written")
    print(f"summary={OUT_SUMMARY}")
    print(
        "expanded="
        f"valid:{expanded['valid_day_count']} "
        f"accept:{expanded['accept_day_count']} "
        f"review:{expanded['review_day_count']} "
        f"unique:{expanded['unique_recipe_count']} "
        f"repeated:{expanded['repeated_recipe_count']} "
        f"loss:{expanded['multi_day_loss']} "
        f"new_selected:{expanded['round37_recipe_selected_count']}"
    )
...
def summary_row(
    ...
    round37_ready_ids: set[str],
) -> dict[str, Any]:
    summary = plan.get("multi_day_summary", {})
    selected_ids = selected_recipe_ids(plan)
    selected_new = sorted(round37_ready_ids.intersection(selected_ids))
    ...
    return {
        ...
        "round37_recipe_selected_count": len(selected_new),
        "round37_recipe_ids_selected": ";".join(selected_new),
        "dominant_recipe_count": dominant_recipe_count(selected_ids),
    }
...
def day_output_rows(
    ...
    for day in plan.get("days", []):
        ...
        day_new = [
            meal.get("recipe_id")
            for meal in meals
            if meal.get("recipe_id") in round37_ready_ids
        ]
        ...
        rows.append(
            {
                ...
                "round37_recipe_ids_selected": ";".join(str(item) for item in day_new),
            }
        )
    return rows
...
def meal_output_rows(
    ...
    for row in multi_day_meal_rows(plan):
        ...
        rows.append(
            {
                "scenario": scenario["scenario"],
                "is_round37_recipe": recipe_id in round37_ready_ids,
                **row,
            }
        )
    return rows
...
def candidate_pool_row(
    ...
    pool_summary = plan.get("candidate_day_pool_summary", {})
    selector = plan.get("selector_diagnostics", {})
    return {
        "scenario": scenario["scenario"],
        "dataset_profile": scenario["dataset_profile"],
        "runtime_seconds": diagnostics["runtime_seconds"],
        "candidate_day_pool_count": pool_summary.get("candidate_day_pool_count"),
        "valid_candidate_day_count": pool_summary.get("valid_candidate_day_count"),
        "accept_candidate_day_count": pool_summary.get("accept_candidate_day_count"),
        "review_candidate_day_count": pool_summary.get("review_candidate_day_count"),
        "reject_candidate_day_count": pool_summary.get("reject_candidate_day_count"),
        "combinations_evaluated": selector.get("combinations_evaluated"),
        "feasible_no_repeat_combinations": selector.get("feasible_no_repeat_combinations"),
        "day_candidate_builder": selector.get("day_candidate_builder"),
        "direct_slot_shortlist_size": selector.get("direct_slot_shortlist_size"),
    }
...
def dominant_recipe_count(selected_ids: set[str]) -> int:
    anchors = {
        "recipes_v1_1_candidate_094",
        "recipes_v1_1_candidate_096",
        "recipes_v1_1_candidate_169",
    }
    return len(anchors.intersection(selected_ids))


def load_round37_ready_recipe_ids() -> set[str]:
    if not ROUND37_CACHE.exists():
        return set()
    df = pd.read_csv(ROUND37_CACHE)
    return set(
        df.loc[
            df["generator_ready_candidate"].astype(str).str.lower() == "true",
            "recipe_id_candidate",
        ].astype(str)
    )


def build_summary_text(rows: list[dict[str, Any]], round37_ready_ids: set[str]) -> str:
    lines = [
        "Generator v1 Round37 expanded dataset impact summary",
        "",
        f"- round37_ready_recipe_count: {len(round37_ready_ids)}",
        f"- round37_ready_recipe_ids: {', '.join(sorted(round37_ready_ids))}",
        "",
        "Comparison",
    ]
    for row in rows:
        lines.append(
            (
                ...
                f"round37_selected={row['round37_recipe_selected_count']}, "
    ...
    lines.extend(
        [
            ...
            "- If the expanded dataset improves variety without repeats, keep it as draft testing data.",
            "- If few new recipes are selected, the next step is repair/mapping coverage, not selector complexity.",
            "",
            "Output files",
            f"- days: {OUT_DAYS}",
            f"- meals: {OUT_MEALS}",
            f"- repetition: {OUT_REPETITION}",
            f"- candidate_pool: {OUT_CANDIDATE_POOL}",
        ]
    )
    return "\n".join(lines) + "\n"
