...
RECIPES = REPO_ROOT / "data/recipesdb/draft/recipes_v1_1_round26_targeted_plus10.csv"
PARSED = REPO_ROOT / "data/recipesdb/draft/recipes_v1_1_round26_plus10_ingredients_parsed.csv"
FOODDB = REPO_ROOT / "data/fooddb/draft/fooddb_v1_1_core_master_draft_round9.csv"
OLD_DATASET_DIR = (
    REPO_ROOT
    / "data/recipesdb/draft/v1_1_generator_ready_slot_checked_time_enriched_snack_curated"
)
PLUS10_DATASET_DIR = (
    REPO_ROOT
...
OUT_UNIT_RULES = (
    REPO_ROOT / "data/recipesdb/draft/recipes_v1_1_round26_plus10_ingredients_unit_rules.csv"
)
OUT_MATCHES = REPO_ROOT / "data/recipesdb/draft/recipes_v1_1_round26_plus10_food_matches.csv"
OUT_UNMAPPED = REPO_ROOT / "data/recipesdb/draft/recipes_v1_1_round26_plus10_unmapped.csv"
OUT_MAPPING_SUMMARY = (
    REPO_ROOT / "data/recipesdb/audit/recipes_v1_1_round26_plus10_mapping_summary.txt"
)
OUT_CACHE = (
    REPO_ROOT / "data/recipesdb/draft/recipes_v1_1_round26_plus10_nutrition_cache.csv"
)
OUT_NUTRITION_SUMMARY = (
    REPO_ROOT / "data/recipesdb/audit/recipes_v1_1_round26_plus10_nutrition_summary.txt"
)
OUT_NUTRITION_AUDIT = (
    REPO_ROOT / "data/recipesdb/audit/recipes_v1_1_round26_plus10_nutrition_audit.csv"
)
OUT_MATERIALIZATION_SUMMARY = (
    REPO_ROOT / "data/recipesdb/audit/recipes_v1_1_round26_plus10_materialization_summary.txt"
)

ROUND26_TAG = "round26_plus10_targeted_lunch_dinner"
PLUS10_SCOPE_STATUS = "v1_1_generator_ready_draft"
PLUS10_NUTRITION_BASIS = "recipes_v1_1_round26_plus10_mapped_ingredients_draft"
PLUS10_CACHE_VERSION = "recipes_v1_1_round26_plus10_001"
...
NUTRITION_AUDIT_COLUMNS = ROUND26_CACHE_COLUMNS + [
    "selected_for_plus10_dataset",
]
...
def main() -> None:
    ...
    unit_rows = apply_round26_unit_rules(parsed_rows)
    ...
    materialize_plus10_dataset(recipes, mapping_rows, cache_rows, ready_ids)
    ...
    write_csv(OUT_CACHE, cache_rows, ROUND26_CACHE_COLUMNS)
    ...
    print("Round26 +10 unit rules, mapping, nutrition cache and materialization written")
    ...
    print(f"plus10_dataset={PLUS10_DATASET_DIR}")
...
def apply_round26_unit_rules(rows: list[dict[str, str]]) -> list[dict[str, object]]:
    result = []
    for row in rows:
        updated = dict(row)
        ...
        if old_grams is None:
            ...
            if grams is not None:
                ...
                updated["parse_notes"] = append_note(notes, "round26_unit_rule_applied")
...
def infer_missing_grams(row: dict[str, str]) -> tuple[float | None, str]:
    ...
    for (term, unit_key), grams_per_unit in UNIT_GRAMS.items():
        if unit == unit_key and (term in name or term in raw):
            return round(quantity * grams_per_unit, 2), f"round26_{term}_{unit_key}_rule"
    return None, ""
...
def infer_count_based_grams(
    ...
    if unit == "clove" and "garlic" in text:
        return round(quantity * 3.0, 2), "round26_garlic_clove_rule"
    if unit in {"count", "head"}:
        if "chicken breast" in text:
            return round(quantity * 150.0, 2), "round26_chicken_breast_count_rule"
        if "potato" in text:
            grams_each = 150.0 if "small" in text else 170.0
            return round(quantity * grams_each, 2), "round26_potato_count_rule"
        if "carrot" in text:
            grams_each = 70.0 if "large" in text else 60.0
            return round(quantity * grams_each, 2), "round26_carrot_count_rule"
        if "onion" in text:
            grams_each = 75.0 if "half" in raw or quantity < 1 else 110.0
            return round(quantity * grams_each, 2), "round26_onion_count_rule"
        if "pepper" in text and ("green" in text or "bell" in text):
            return round(quantity * 120.0, 2), "round26_green_pepper_count_rule"
        if "cabbage" in text:
            return round(quantity * 900.0, 2), "round26_cabbage_head_rule"
        if "egg" in text:
            return round(quantity * 50.0, 2), "round26_egg_count_rule"
        if "lemon" in text:
            return round(quantity * 58.0, 2), "round26_lemon_count_rule"
    return None
...
def infer_packaged_grams(
    ...
    if unit in {"can", "cans"} and "beans" in text and "drained" in raw:
        return round(quantity * 255.0, 2), "round26_drained_bean_can_rule"
    match = re.search(r"([0-9]+(?:\.[0-9]+)?)\s*(?:fluid\s+)?ounce", raw)
    if not match:
        return None
    ounce_value = float(match.group(1))
    return round(quantity * ounce_value * 28.3495, 2), "round26_package_ounce_rule"
...
def map_ingredient(
    ...
    if parse_status == "failed_parse":
        return mapping_result("", "", "unmapped", "low", "parse_failed", "round26_parse_failed")
    ...
    if food_id and food_id in food_lookup:
        return mapping_result(
            ...
            "accepted_auto",
            "high",
            "round26_existing_fooddb_alias",
            "round26_targeted_existing_fooddb_item",
        )
...
def food_id_for_name(name: str, raw: str) -> str:
    ...
    for term, food_id in CONTAINS_ALIAS_FOOD_IDS:
        if term in name or term in raw:
            return food_id
    if name in ROUND26_ALIAS_FOOD_IDS:
        return ROUND26_ALIAS_FOOD_IDS[name]
    return ""
...
def mapping_result(
    ...
    return {
        ...
        "method": method,
        "notes": notes,
        "manual_notes": "round26_no_fooddb_rows_added",
    }
...
def build_cache_rows(
    ...
    for recipe in recipes:
        ...
        row = {
            ...
            "nutrition_basis": PLUS10_NUTRITION_BASIS,
            "servings_basis": format_number(servings),
            "uses_pilot_servings_fallback": "True",
            "servings_estimation_method": "round26_complete_main_default",
            "servings_estimation_reasons": "complete_lunch_dinner_default_4_servings",
            ...
            "cache_version": PLUS10_CACHE_VERSION,
        }
        cache_rows.append(row)
        audit_row = dict(row)
        audit_row["selected_for_plus10_dataset"] = str(ready).lower()
...
def generator_ready_status(
    ...
    if macro_ratio < 0.40:
        reasons.append("macro_relevant_mapped_weight_ratio_lt_0.40")
    if energy_per < 250:
        reasons.append("kcal_per_serving_lt_250")
    if protein_per < 14:
        reasons.append("protein_per_serving_lt_14")
    if carbs_per < 20:
        reasons.append("carbs_per_serving_lt_20")
...
def materialize_plus10_dataset(
    recipes: list[dict[str, str]],
    mapping_rows: list[dict[str, object]],
    cache_rows: list[dict[str, object]],
    ready_ids: set[str],
) -> None:
    if PLUS10_DATASET_DIR.exists():
        for child in PLUS10_DATASET_DIR.iterdir():
            if child.is_file():
                child.unlink()
    else:
        PLUS10_DATASET_DIR.mkdir(parents=True, exist_ok=True)
    ...
    write_csv(
        PLUS10_DATASET_DIR / "recipes.csv",
        old_recipes + plus_recipe_rows,
        list(old_recipes[0].keys()),
    )
    write_csv(
        PLUS10_DATASET_DIR / "recipe_ingredients.csv",
        old_ingredients + plus_ingredient_rows,
        list(old_ingredients[0].keys()),
    )
    write_csv(
        PLUS10_DATASET_DIR / "recipe_nutrition_cache.csv",
        old_cache + plus_cache_rows,
        list(old_cache[0].keys()),
    )
    readme_text = "\n".join(
        [
            "Recipes_DB v1.1 generator-ready snack-curated + Round26 plus10 draft",
            "",
            "Draft/test only. Do not treat as current production data.",
            f"Base dataset: {OLD_DATASET_DIR}",
            f"Round26 ready additions: {len(ready_ids)}",
            f"QC tag: {ROUND26_TAG}",
            "",
        ]
    )
    (PLUS10_DATASET_DIR / "README").write_text(readme_text, encoding="utf-8")
    (PLUS10_DATASET_DIR / "README.txt").write_text(readme_text, encoding="utf-8")
...
def materialized_recipe_row(recipe: dict[str, str], cache: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "source_dataset": "recipes_dataset_64k_dishes_round26_plus10",
        ...
        "difficulty_level": "",
        "scope_status": PLUS10_SCOPE_STATUS,
        "has_ingredients_parsed": 1,
        "has_nutrition_cache": 1,
        "is_pilot_recipe": 0,
        "is_active": 1,
        "qc_recipe_status": "generator_ready_draft",
        "qc_notes": f"{ROUND26_TAG}; draft_only; no_fooddb_rows_added",
        "allowed_slots_json": json.dumps(["lunch", "dinner"]),
        "slot_policy_reason": "round26_targeted_complete_main_lunch_dinner",
        ...
        "time_estimation_method": "round26_direction_step_estimate",
        "time_estimation_reasons": "draft_plus10_direction_count_estimate",
    }
...
def materialized_ingredient_row(row: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "qc_ingredient_status": "accepted_auto_round26"
        if row.get("mapping_status") == "accepted_auto"
        else "needs_review_round26",
        "qc_notes": ROUND26_TAG,
    }
...
def materialized_cache_row(row: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "cache_version": PLUS10_CACHE_VERSION,
        "qc_notes": f"{ROUND26_TAG}; {row['generator_ready_failure_reason']}",
...
def build_mapping_summary(rows: list[dict[str, object]]) -> str:
    ...
    lines = [
        "Recipes_DB v1.1 Round26 +10 mapping summary",
        "",
    ...
    lines.extend(
        [
            "",
            "Strict note:",
            "Round26 only maps to existing Food_DB v1.1 round9 draft rows; no Food_DB rows were added.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def build_nutrition_summary(rows: list[dict[str, object]]) -> str:
    ...
    lines = [
        "Recipes_DB v1.1 Round26 +10 nutrition summary",
        "",
        f"recipes={len(rows)}",
...
def build_materialization_summary(
    ...
    lines = [
        "Recipes_DB v1.1 Round26 +10 materialization summary",
        "",
        f"old_dataset={OLD_DATASET_DIR}",
        f"new_dataset={PLUS10_DATASET_DIR}",
        f"old_recipe_count={old_count}",
        f"selected_plus10_count={len(recipes)}",
        f"generator_ready_plus10_count={len(ready_ids)}",
