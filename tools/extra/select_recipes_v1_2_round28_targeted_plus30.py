...
ROUND27_RECOMMENDATIONS = Path(
    "data/recipesdb/audit/generator_v1_round27_next_expansion_recommendations.csv"
)
ROUND27_SLOT_DEFICITS = Path("data/recipesdb/audit/generator_v1_round27_slot_deficit_audit.csv")
ROUND27_ROOT_CAUSES = Path("data/recipesdb/audit/generator_v1_round27_repeated_recipe_root_causes.csv")

OUT_CANDIDATES = Path("data/recipesdb/draft/recipes_v1_2_round28_targeted_plus30_candidates.csv")
OUT_SELECTED = Path("data/recipesdb/draft/recipes_v1_2_round28_targeted_plus30.csv")
OUT_SUMMARY = Path("data/recipesdb/audit/recipes_v1_2_round28_targeted_plus30_summary.txt")
OUT_EXCLUSION = Path("data/recipesdb/audit/recipes_v1_2_round28_targeted_plus30_exclusion_log.csv")
OUT_QUALITY_AUDIT = Path("data/recipesdb/audit/recipes_v1_2_round28_targeted_plus30_quality_audit.csv")
...
TARGET_QUOTAS = {
    "breakfast_competitor": 8,
    "carb_protein_main": 14,
    "fish_turkey_pork_legume_main": 8,
}
...
def main() -> None:
    ...
    plus10_rows = read_csv(PLUS10_RECIPES)
    exclusion_indices, exclusion_titles = existing_recipe_keys(curated_rows, plus10_rows)
    ...
    selected = select_targeted_plus30(candidate_pool)
    ...
    print("Round28 targeted v1.2 +30 selection written")
...
def choose_target_bucket(
    ...
    if primary_protein in {"fish", "turkey", "pork", "vegetarian"} and has_carb and (has_veg or primary_protein == "vegetarian"):
        return "fish_turkey_pork_legume_main"
    if primary_protein and has_carb and (has_veg or primary_protein in {"chicken", "beef"}):
...
def selection_score(
    ...
    if target_bucket == "breakfast_competitor":
        score += 15
    if target_bucket == "fish_turkey_pork_legume_main":
        score += 12
    if primary_protein:
        score += 20
...
def expected_generator_value(target_bucket: str, primary_protein: str) -> str:
    ...
    if primary_protein in {"fish", "turkey", "pork", "vegetarian"}:
        return "lunch_dinner_variety"
    return "cabbage_alternative"
...
def select_targeted_plus30(candidates: list[dict[str, object]]) -> list[dict[str, object]]:
...
def finalize_selected_row(index: int, row: dict[str, object]) -> dict[str, object]:
    finalized = dict(row)
    finalized["recipe_id_candidate"] = f"recipes_v1_2_round28_plus30_{index:03d}"
    return finalized


def finalize_candidate_pool_row(row: dict[str, object], index: int) -> dict[str, object]:
    finalized = dict(row)
    finalized["recipe_id_candidate"] = f"round28_candidate_pool_{index:03d}"
    return finalized
...
def build_summary(
    ...
    lines = [
        "Recipes_DB v1.2 Round28 targeted +30 selection",
        "",
        f"source={SOURCE_RECIPES}",
        f"curated_200={CURATED_200}",
        f"plus10_dataset={PLUS10_RECIPES}",
        f"round27_recommendations_available={ROUND27_RECOMMENDATIONS.exists()}",
        f"round27_slot_deficits_available={ROUND27_SLOT_DEFICITS.exists()}",
        f"round27_root_causes_available={ROUND27_ROOT_CAUSES.exists()}",
        "",
    ...
    lines.extend(
        [
            "",
            "Strict note:",
            "Round28 is targeted draft data only; weak or review-level recipes are not forced into the final selection.",
        ]
    )
    return "\n".join(lines) + "\n"
