...
def _profile_request(enabled: bool = True) -> dict[str, Any]:
    return {
        ...
        "display_name": "Blood Pressure Alex",
        "age": 38,
        "sex": "male",
        "weight_kg": 82.0,
        "height_cm": 180.0,
...
def _scoring_checks() -> list[str]:
    ...
    preferences = {
        "dietary_patterns": {},
        "health_modes": {"hypertension_friendly": True},
    }
    salty_processed = compute_health_and_diet_fit(
        {"salt_g": 2.4, "health_proxy_flags": ["processed_salty", "salty_sauce"]},
        preferences,
    )
    simple_recipe = compute_health_and_diet_fit(
        {"salt_g": 0.4, "health_proxy_flags": []},
        preferences,
    )
    if float(salty_processed.get("health_and_diet_fit") or 1.0) >= float(
        simple_recipe.get("health_and_diet_fit") or 0.0
    ):
        errors.append("hypertension_friendly_salty_proxy_not_penalized")
    reasons = salty_processed.get("health_and_diet_reasons")
    if not isinstance(reasons, list) or not any(
        "hypertension_friendly" in item for item in reasons
    ):
        errors.append("hypertension_friendly_reasons_missing")
    return errors
...
def _generation_check(profile_payload: dict[str, Any]) -> list[str]:
    ...
    if not meals:
        ...
        return errors
    first_meal = meals[0]
    if "hypertension_friendly" not in first_meal.get("active_health_modes", []):
        errors.append("meal_active_health_modes_missing_hypertension_friendly")
    return errors
...
def _mobile_checks() -> list[str]:
    ...
    for marker in ("treat hypertension", "treat blood pressure", "diagnose", "cure"):
...
def main() -> int:
    ...
    if not isinstance(modes, dict) or modes.get("hypertension_friendly") is not True:
        errors.append("profile_hypertension_friendly_not_returned")
    ...
    summary_lines = [
        "Hypertension-friendly profile summary",
