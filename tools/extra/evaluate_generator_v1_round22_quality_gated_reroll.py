...
OUT_SUMMARY = OUT_DIR / "generator_v1_round22_quality_gated_reroll_summary.txt"
OUT_RUNS = OUT_DIR / "generator_v1_round22_quality_gated_reroll_runs.csv"
OUT_MEALS = OUT_DIR / "generator_v1_round22_quality_gated_reroll_meals.csv"
...
def main() -> None:
    ...
    run_specs = [
        {
            ...
            "diversity_mode_requested": "none",
        },
        {
            "run_number": 2,
            "run_label": "RUN 2 - Generate varied with quality gate",
            "generation_trigger": "varied",
            "diversity_mode_requested": "avoid_recent",
        },
        {
            "run_number": 3,
            "run_label": "RUN 3 - Generate varied with quality gate",
            "generation_trigger": "varied",
            "diversity_mode_requested": "avoid_recent",
        },
    ]
    ...
    for spec in run_specs:
        ...
        if spec["generation_trigger"] == "varied":
            plan = select_varied_plan(context, recent_ids)
        else:
            plan = select_best_plan(context)
    ...
    print("Generator v1 round22 quality-gated reroll evaluation written")
    ...
    for row in run_rows:
        print(
            ...
            f"mode={row['diversity_mode_used']} "
            f"gate={row['quality_gate_status']} "
...
def select_best_plan(context: dict[str, Any]) -> dict[str, Any]:
    target = context["target"]
    plan = select_one_day_plan_balanced(
        slot_candidates_by_slot=context["slot_candidates_by_slot"],
        target=target,
        slot_order=context["slot_order"],
        config=selector_config("none", []),
    )
    plan["quality_gate_fallback_used"] = False
    plan["quality_gate_selected_mode"] = "none"
    plan["quality_gate_reroll_diagnostics"] = {
        "attempted_modes": [],
        "selected_mode": "none",
        "fallback_used": False,
        "recent_recipe_count_used": 0,
    }
    return plan


def select_varied_plan(
    context: dict[str, Any],
    recent_recipe_ids: set[str],
) -> dict[str, Any]:
    return select_quality_gated_reroll(
        slot_candidates_by_slot=context["slot_candidates_by_slot"],
        target=context["target"],
        slot_order=context["slot_order"],
        recent_recipe_ids=recent_recipe_ids,
        base_config=selector_config("avoid_recent", sorted(recent_recipe_ids)),
    )
...
def attach_run_metadata(
    ...
    plan.setdefault("quality_gate_fallback_used", False)
    plan.setdefault("quality_gate_selected_mode", spec["diversity_mode_requested"])
    plan["run_number"] = spec["run_number"]
    plan["run_label"] = spec["run_label"]
    plan["generation_trigger"] = spec["generation_trigger"]
    plan["diversity_mode_requested"] = spec["diversity_mode_requested"]
    plan["diversity_mode_used"] = plan.get(
        "quality_gate_selected_mode",
        spec["diversity_mode_requested"],
    )
...
def run_row(
    ...
    return {
        ...
        "diversity_mode_requested": plan["diversity_mode_requested"],
        "diversity_mode_used": plan["diversity_mode_used"],
        "quality_gate": QUALITY_GATE,
        "quality_gate_status": plan.get("quality_gate_status"),
        "quality_gate_reasons": serialize(plan.get("quality_gate_reasons")),
        "quality_gate_score": plan.get("quality_gate_score"),
        "fallback_used": plan.get("quality_gate_fallback_used"),
        "attempted_modes": serialize_json(attempted_modes(plan)),
        ...
        rows.append(
            {
                ...
                "quality_gate_status": plan.get("quality_gate_status"),
                "fallback_used": plan.get("quality_gate_fallback_used"),
                "diversity_mode_used": plan.get("diversity_mode_used"),
...
def build_summary(
    ...
    run2 = run_rows[1]
    run3 = run_rows[2]
    fallback_count = sum(str(row["fallback_used"]) == "True" for row in run_rows)
    bad_selected_count = sum(row["verdict"] == "bad" for row in run_rows)
    bad_varied_prevented = (
        str(run3["fallback_used"]) == "True"
        or run3["quality_gate_status"] != "reject"
        and run3["verdict"] != "bad"
    )
    lines = [
        "Generator v1 round22 quality-gated reroll",
        "",
        "Config:",
        ...
        "- reject: invalid plan, adjusted_day_loss > 0.20, base_day_loss > 0.18",
        "- reject: kcal ratio outside 0.75..1.25, protein ratio < 0.75, carbs ratio < 0.60",
        "- reject: breakfast_too_large, unrealistic_large_portion, snack_too_large",
        "- reject: two or more lunch/dinner realism issues",
        "- review: adjusted_day_loss 0.12..0.20, carbs ratio 0.60..0.75, one main realism issue, borderline_large_portion",
        "",
        "Runs:",
    ]
    for row in run_rows:
        lines.extend(
            [
                f"{row['run_label']}",
                f"- requested_mode={row['diversity_mode_requested']}",
                f"- selected_mode={row['diversity_mode_used']}",
                f"- quality_gate_status={row['quality_gate_status']}",
                f"- quality_gate_reasons={row['quality_gate_reasons']}",
    ...
    lines.extend(
        [
            "Round21 comparison:",
            "- Round21 Run 2/3 proved that blind avoid_recent can produce valid but weak or bad menus.",
            f"- Round22 Run 2 fallback_used={run2['fallback_used']} selected_mode={run2['diversity_mode_used']}.",
            f"- Round22 Run 3 fallback_used={run3['fallback_used']} selected_mode={run3['diversity_mode_used']}.",
            f"- bad_varied_run3_prevented={bad_varied_prevented}",
            f"- fallback_count={fallback_count}",
            f"- bad_selected_count={bad_selected_count}",
            "",
            "Implication:",
            "- Diversity is useful only behind a quality gate.",
            "- If the gate falls back to best, the result is realistic but not varied.",
            "- Multi-day v1 should use the same pattern: try variety, reject bad degradation, report fallback honestly.",
...
def verdict_for_plan(plan: dict[str, Any], overlap_last2: set[str]) -> str:
    ...
    if plan.get("quality_gate_status") == "reject":
        return "bad"
    if flags & severe_flags or any(
        ...
        return "bad"
    if bool(plan.get("quality_gate_fallback_used")):
        return "human-realistic but not varied"
    if plan.get("quality_gate_status") == "review" or flags or overlap_last2:
        return "needs review"
    for meal in plan.get("selected_meals", []):
...
def attempted_modes(plan: dict[str, Any]) -> list[dict[str, Any]]:
    diagnostics = plan.get("quality_gate_reroll_diagnostics", {})
    if not isinstance(diagnostics, dict):
        return []
    value = diagnostics.get("attempted_modes", [])
            ...
            rows.append(
                {
                    ...
                    "run_id": f"round22-{plan.get('run_number')}",
                    "slot": meal.get("slot"),
                    "recipe_id": recipe_id,
                    "display_name": meal.get("display_name"),
                }
