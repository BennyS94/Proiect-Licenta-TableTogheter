...
OUT_SUMMARY = OUT_DIR / "generator_v1_round28_plus30_multiday_impact_summary.txt"
OUT_DAYS = OUT_DIR / "generator_v1_round28_plus30_multiday_days.csv"
OUT_MEALS = OUT_DIR / "generator_v1_round28_plus30_multiday_meals.csv"
OUT_REPETITION = OUT_DIR / "generator_v1_round28_plus30_multiday_repetition.csv"

PORTION_POLICY = "target_aware"
MEAL_REALISM_MODE = "practical"
QUALITY_GATE = "demo_safe"
DAYS = 3
ROUND28_PREFIX = "recipes_v1_2_round28_plus30_"
...
def main() -> None:
    ...
    print("Generator v1 Round28 plus30 multi-day impact evaluation written")
        ...
        rows.append(
            {
                ...
                "dataset_profile": dataset["dataset_profile"],
                "multi_day_mode": MULTI_DAY_MODE_GLOBAL,
                "is_round28_added_recipe": recipe_id.startswith(ROUND28_PREFIX),
...
def build_summary(
    ...
    old = summaries["round26_plus10"]
    plus = summaries["round28_plus30"]
    comparison = compare_summaries(old, plus, meal_rows, repetition_rows)
    lines = [
        "Generator v1 Round28 plus30 multi-day impact evaluation",
        "",
        "selection_mode=balanced_day",
    ...
    for dataset, plan, context in plans:
        ...
        lines.extend(
            [
                ...
                f"round28_added_recipe_meal_count={added_recipe_meal_count(meal_rows, str(dataset['label']))}",
                "round28_added_recipes_selected=" + format_list(added_selected),
    ...
    lines.extend(
        [
            ...
            f"round28_added_recipe_meal_count={comparison['round28_added_recipe_meal_count']}",
            f"plus30_improved_variety={comparison['plus30_improved_variety']}",
            f"plus30_destroyed_quality={comparison['plus30_destroyed_quality']}",
            "",
            "Daily macro fit",
        ]
    )
...
def compare_summaries(
    ...
    old_label = "round26_plus10"
    plus_label = "round28_plus30"
    quality_worse = (
    ...
    return {
        ...
        "round28_added_recipe_meal_count": added_recipe_meal_count(meal_rows, plus_label),
        "plus30_improved_variety": variety_better,
        "plus30_destroyed_quality": quality_worse,
    }
...
def strict_conclusion(
    ...
    if comparison.get("plus30_destroyed_quality"):
        return "Plus30 is not acceptable as an improvement: it reduces daily quality."
    if comparison.get("round28_added_recipe_meal_count", 0) <= 0:
        return "Plus30 did not solve the bottleneck: no added recipe was selected."
    if comparison.get("plus30_improved_variety") and plus_summary.get("multi_day_classification") == "multi_day_good":
        return "Plus30 improves variety enough for draft testing, but it is still draft data."
    if comparison.get("plus30_improved_variety"):
        return "Plus30 improves variety partially, but the multi-day plan still needs review."
    return "Plus30 is inconclusive: added recipes exist, but the selector still concentrates the plan."
...
def selected_added_recipe_names(
    ...
    names = {
        ...
        and bool(row.get("is_round28_added_recipe"))
    }
