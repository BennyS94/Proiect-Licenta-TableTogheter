...
RECIPES = REPO_ROOT / "data/recipesdb/draft/recipes_v1_2_round30_targeted_plus15.csv"
PARSED = REPO_ROOT / "data/recipesdb/draft/recipes_v1_2_round30_plus15_ingredients_parsed.csv"
...
PLUS15_DATASET_DIR = (
    REPO_ROOT
    / "data/recipesdb/draft/v1_2_generator_ready_plus30_plus15"
)

OUT_UNIT_RULES = (
    REPO_ROOT / "data/recipesdb/draft/recipes_v1_2_round30_plus15_ingredients_unit_rules.csv"
)
OUT_MATCHES = REPO_ROOT / "data/recipesdb/draft/recipes_v1_2_round30_plus15_food_matches.csv"
OUT_UNMAPPED = REPO_ROOT / "data/recipesdb/draft/recipes_v1_2_round30_plus15_unmapped.csv"
OUT_MAPPING_SUMMARY = (
    REPO_ROOT / "data/recipesdb/audit/recipes_v1_2_round30_plus15_mapping_summary.txt"
)
OUT_MAPPING_REVIEW = (
    REPO_ROOT / "data/recipesdb/audit/recipes_v1_2_round30_plus15_mapping_review.csv"
)
OUT_MAPPING_QUALITY_AUDIT = (
    REPO_ROOT / "data/recipesdb/audit/recipes_v1_2_round30_plus15_mapping_quality_audit.csv"
)
OUT_CACHE = (
    REPO_ROOT / "data/recipesdb/draft/recipes_v1_2_round30_plus15_nutrition_cache.csv"
)
OUT_NUTRITION_SUMMARY = (
    REPO_ROOT / "data/recipesdb/audit/recipes_v1_2_round30_plus15_nutrition_summary.txt"
)
OUT_NUTRITION_AUDIT = (
    REPO_ROOT / "data/recipesdb/audit/recipes_v1_2_round30_plus15_nutrition_audit.csv"
)
OUT_GENERATOR_READY_AUDIT = (
    REPO_ROOT / "data/recipesdb/audit/recipes_v1_2_round30_plus15_generator_ready_audit.csv"
)
OUT_MATERIALIZATION_SUMMARY = (
    REPO_ROOT / "data/recipesdb/audit/recipes_v1_2_round30_plus15_materialization_summary.txt"
)
OUT_MATERIALIZATION_AUDIT = (
    REPO_ROOT / "data/recipesdb/audit/recipes_v1_2_round30_plus15_materialization_audit.csv"
)

ROUND30_TAG = "round30_plus15_targeted_expansion"
PLUS15_SCOPE_STATUS = "v1_2_generator_ready_draft"
PLUS15_NUTRITION_BASIS = "recipes_v1_2_round30_plus15_mapped_ingredients_draft"
PLUS15_CACHE_VERSION = "recipes_v1_2_round30_plus15_001"
...
NUTRITION_AUDIT_COLUMNS = ROUND30_CACHE_COLUMNS + [
    "selected_for_plus15_dataset",
]
...
def main() -> None:
    ...
    materialize_plus15_dataset(recipes, mapping_rows, cache_rows, ready_ids)
    ...
    write_csv(OUT_CACHE, cache_rows, ROUND30_CACHE_COLUMNS)
    ...
    print("Round30 +15 unit rules, mapping, nutrition cache and materialization written")
    ...
    print(f"plus15_dataset={PLUS15_DATASET_DIR}")
...
def apply_round30_unit_rules(rows: list[dict[str, str]]) -> list[dict[str, object]]:
    result = []
    for row in rows:
        updated = dict(row)
        ...
        if old_grams is None:
            ...
            if grams is not None:
                ...
                updated["parse_notes"] = append_note(notes, "round30_unit_rule_applied")
...
def infer_missing_grams(row: dict[str, str]) -> tuple[float | None, str]:
    ...
    for (term, unit_key), grams_per_unit in UNIT_GRAMS.items():
        if unit == unit_key and (term in name or term in raw):
            return round(quantity * grams_per_unit, 2), f"round30_{term}_{unit_key}_rule"
    return None, ""
...
def infer_count_based_grams(
    ...
    if unit == "clove" and "garlic" in text:
        return round(quantity * 3.0, 2), "round30_garlic_clove_rule"
    if unit in {"count", "head"}:
        if "chicken breast" in text:
            return round(quantity * 150.0, 2), "round30_chicken_breast_count_rule"
        if "potato" in text:
            grams_each = 150.0 if "small" in text else 170.0
            return round(quantity * grams_each, 2), "round30_potato_count_rule"
        if "carrot" in text:
            grams_each = 70.0 if "large" in text else 60.0
            return round(quantity * grams_each, 2), "round30_carrot_count_rule"
        if "onion" in text:
            grams_each = 75.0 if "half" in raw or quantity < 1 else 110.0
            return round(quantity * grams_each, 2), "round30_onion_count_rule"
        if "pepper" in text and ("green" in text or "bell" in text):
            return round(quantity * 120.0, 2), "round30_green_pepper_count_rule"
        if "cabbage" in text:
            return round(quantity * 900.0, 2), "round30_cabbage_head_rule"
        if "egg" in text:
            return round(quantity * 50.0, 2), "round30_egg_count_rule"
        if "lemon" in text:
            return round(quantity * 58.0, 2), "round30_lemon_count_rule"
        if "banana" in text:
            return round(quantity * 118.0, 2), "round30_banana_count_rule"
        if "apple" in text:
            return round(quantity * 180.0, 2), "round30_apple_count_rule"
        if "slice" in text and "bread" in text:
            return round(quantity * 35.0, 2), "round30_bread_slice_count_rule"
        if "tortilla" in text:
            return round(quantity * 45.0, 2), "round30_tortilla_count_rule"
    return None
...
def infer_packaged_grams(
    ...
    if unit in {"can", "cans"} and "beans" in text and "drained" in raw:
        return round(quantity * 255.0, 2), "round30_drained_bean_can_rule"
    match = re.search(r"([0-9]+(?:\.[0-9]+)?)\s*(?:fluid\s+)?ounce", raw)
    if not match:
        return None
    ounce_value = float(match.group(1))
    return round(quantity * ounce_value * 28.3495, 2), "round30_package_ounce_rule"
...
def map_ingredient(
    ...
    if parse_status == "failed_parse":
        return mapping_result("", "", "unmapped", "low", "parse_failed", "round30_parse_failed")
    ...
    if food_id and food_id in food_lookup:
        return mapping_result(
            ...
            "accepted_auto",
            "high",
            "round30_existing_fooddb_alias",
            "round30_targeted_existing_fooddb_item",
        )
...
def food_id_for_name(name: str, raw: str) -> str:
    ...
    for term, food_id in CONTAINS_ALIAS_FOOD_IDS:
        if term in name or term in raw:
            return food_id
    if name in ROUND30_ALIAS_FOOD_IDS:
        return ROUND30_ALIAS_FOOD_IDS[name]
    return ""
...
def mapping_result(
    ...
    return {
        ...
        "method": method,
        "notes": notes,
        "manual_notes": "round30_no_fooddb_rows_added",
    }
...
def build_cache_rows(
    ...
    for recipe in recipes:
        ...
        row = {
            ...
            "nutrition_basis": PLUS15_NUTRITION_BASIS,
            "servings_basis": format_number(servings),
            "uses_pilot_servings_fallback": "True",
            "servings_estimation_method": "round30_bucket_default",
            ...
            "cache_version": PLUS15_CACHE_VERSION,
        }
        cache_rows.append(row)
        audit_row = dict(row)
        audit_row["selected_for_plus15_dataset"] = str(ready).lower()
...
def generator_ready_status(
    ...
    if bucket == "breakfast_competitor":
        if energy_per < 250:
            reasons.append("breakfast_kcal_per_serving_lt_250")
        if energy_per > 800:
            reasons.append("breakfast_kcal_per_serving_gt_800")
        if protein_per < 10:
            reasons.append("breakfast_protein_per_serving_lt_10")
        if carbs_per < 35:
            reasons.append("breakfast_carbs_per_serving_lt_35")
    else:
        if energy_per < 400:
            reasons.append("main_kcal_per_serving_lt_400")
        if protein_per < 18:
            reasons.append("main_protein_per_serving_lt_18")
        if carbs_per < 35:
            reasons.append("main_carbs_per_serving_lt_35")
...
def materialize_plus15_dataset(
    recipes: list[dict[str, str]],
    mapping_rows: list[dict[str, object]],
    cache_rows: list[dict[str, object]],
    ready_ids: set[str],
) -> None:
    PLUS15_DATASET_DIR.mkdir(parents=True, exist_ok=True)

    old_recipes = read_csv(BASE_DATASET_DIR / "recipes.csv")
    old_ingredients = read_csv(BASE_DATASET_DIR / "recipe_ingredients.csv")
    old_cache = read_csv(BASE_DATASET_DIR / "recipe_nutrition_cache.csv")
    ...
    write_csv(
        PLUS15_DATASET_DIR / "recipes.csv",
        old_recipes + plus_recipe_rows,
        list(old_recipes[0].keys()),
    )
    write_csv(
        PLUS15_DATASET_DIR / "recipe_ingredients.csv",
        old_ingredients + plus_ingredient_rows,
        list(old_ingredients[0].keys()),
    )
    write_csv(
        PLUS15_DATASET_DIR / "recipe_nutrition_cache.csv",
        old_cache + plus_cache_rows,
        list(old_cache[0].keys()),
    )
    readme_text = "\n".join(
        [
            "Recipes_DB v1.2 generator-ready plus30_plus15 draft",
            "",
            "Draft/test only. Do not treat as current production data.",
            f"Base dataset: {BASE_DATASET_DIR}",
            f"Round30 plus15 ready additions: {len(ready_ids)}",
            f"QC tag: {ROUND30_TAG}",
            "",
        ]
    )
    (PLUS15_DATASET_DIR / "README_v1_2_generator_ready_plus30_plus15.txt").write_text(
        readme_text,
        encoding="utf-8",
    )
...
def materialized_recipe_row(recipe: dict[str, str], cache: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "source_dataset": "recipes_dataset_64k_dishes_round30_plus15",
        ...
        "difficulty_level": "",
        "scope_status": PLUS15_SCOPE_STATUS,
        "has_ingredients_parsed": 1,
        "has_nutrition_cache": 1,
        "is_pilot_recipe": 0,
        "is_active": 1,
        "qc_recipe_status": "generator_ready_draft",
        "qc_notes": f"{ROUND30_TAG}; draft_only; no_fooddb_rows_added",
        "allowed_slots_json": json.dumps(allowed_slots),
        "slot_policy_reason": f"round30_targeted_{target_bucket}",
        ...
        "time_estimation_method": "round30_direction_step_estimate",
        "time_estimation_reasons": "draft_plus15_direction_count_estimate",
    }
...
def materialized_ingredient_row(row: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "qc_ingredient_status": "accepted_auto_round30"
        if row.get("mapping_status") == "accepted_auto"
        else "needs_review_round30",
        "qc_notes": ROUND30_TAG,
    }
...
def materialized_cache_row(row: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "cache_version": PLUS15_CACHE_VERSION,
        "qc_notes": f"{ROUND30_TAG}; {row['generator_ready_failure_reason']}",
...
def build_mapping_summary(rows: list[dict[str, object]]) -> str:
    ...
    lines = [
        "Recipes_DB v1.2 Round30 +15 mapping summary",
        "",
    ...
    lines.extend(
        [
            "",
            "Strict note:",
            "Round30 only maps to existing Food_DB v1.1 round9 draft rows; no Food_DB rows were added.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def build_nutrition_summary(rows: list[dict[str, object]]) -> str:
    ...
    lines = [
        "Recipes_DB v1.2 Round30 +15 nutrition summary",
        "",
        f"recipes={len(rows)}",
...
def build_materialization_summary(
    ...
    old_count = len(read_csv(BASE_DATASET_DIR / "recipes.csv"))
    lines = [
        "Recipes_DB v1.2 Round30 +15 materialization summary",
        "",
        f"old_dataset={BASE_DATASET_DIR}",
        f"new_dataset={PLUS15_DATASET_DIR}",
        f"old_recipe_count={old_count}",
        f"selected_plus15_count={len(recipes)}",
        f"generator_ready_plus15_count={len(ready_ids)}",
