...
OUT_SUMMARY = OUT_DIR / "generator_v1_round26_plus10_multiday_impact_summary.txt"
OUT_DAYS = OUT_DIR / "generator_v1_round26_plus10_multiday_days.csv"
OUT_MEALS = OUT_DIR / "generator_v1_round26_plus10_multiday_meals.csv"
OUT_REPETITION = OUT_DIR / "generator_v1_round26_plus10_multiday_repetition.csv"

PORTION_POLICY = "target_aware"
MEAL_REALISM_MODE = "practical"
QUALITY_GATE = "demo_safe"
DAYS = 3
ROUND26_PREFIX = "recipes_v1_1_round26_plus10_"
...
def main() -> None:
    ...
    print("Generator v1 Round26 plus10 multi-day impact evaluation written")
        ...
        rows.append(
            {
                ...
                "dataset_profile": dataset["dataset_profile"],
                "multi_day_mode": MULTI_DAY_MODE_GLOBAL,
                "is_round26_added_recipe": recipe_id.startswith(ROUND26_PREFIX),
...
def build_summary(
    ...
    old = summaries["old_snack_curated"]
    plus = summaries["round26_plus10"]
    comparison = compare_summaries(old, plus, meal_rows, repetition_rows)
    lines = [
        "Generator v1 Round26 plus10 multi-day impact evaluation",
        "",
        "selection_mode=balanced_day",
    ...
    for dataset, plan, context in plans:
        ...
        lines.extend(
            [
                ...
                f"round26_added_recipe_meal_count={added_recipe_meal_count(meal_rows, str(dataset['label']))}",
                "round26_added_recipes_selected=" + format_list(added_selected),
    ...
    lines.extend(
        [
            ...
            f"round26_added_recipe_meal_count={comparison['round26_added_recipe_meal_count']}",
            f"plus10_improved_variety={comparison['plus10_improved_variety']}",
            f"plus10_destroyed_quality={comparison['plus10_destroyed_quality']}",
            "",
            "Daily macro fit",
        ]
    )
...
def compare_summaries(
    ...
    old_label = "old_snack_curated"
    plus_label = "round26_plus10"
    quality_worse = (
    ...
    return {
        ...
        "round26_added_recipe_meal_count": added_recipe_meal_count(meal_rows, plus_label),
        "plus10_improved_variety": variety_better,
        "plus10_destroyed_quality": quality_worse,
    }
...
def strict_conclusion(
    ...
    if comparison.get("plus10_destroyed_quality"):
        return "Plus10 is not acceptable as an improvement: it reduces daily quality."
    if comparison.get("round26_added_recipe_meal_count", 0) <= 0:
        return "Plus10 did not solve the bottleneck: no added recipe was selected."
    if comparison.get("plus10_improved_variety") and plus_summary.get("multi_day_classification") == "multi_day_good":
        return "Plus10 improves variety enough for draft testing, but it is still draft data."
    if comparison.get("plus10_improved_variety"):
        return "Plus10 improves variety partially, but the multi-day plan still needs review."
    return "Plus10 is inconclusive: added recipes exist, but the selector still concentrates the plan."
...
def selected_added_recipe_names(
    ...
    names = {
        ...
        and bool(row.get("is_round26_added_recipe"))
    }
